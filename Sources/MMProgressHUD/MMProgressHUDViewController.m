...
- (BOOL)oldRootViewControllerShouldRotateToOrientation:(UIInterfaceOrientation)toInterfaceOrientation {
    ...
    if ([[self superclass] instancesRespondToSelector:@selector(presentedViewController)] &&
        ...
        shouldRotateToOrientation = [rootViewController.presentedViewController shouldAutorotate];
    }
    
    if ((shouldRotateToOrientation == NO) &&
        (rootViewController != nil)) {
        shouldRotateToOrientation = [rootViewController shouldAutorotate];
    }
        ...
        shouldRotateToOrientation = [super shouldAutorotate];
    }
...
#warning 改出之前从管理计划会出现崩溃问题   现在暂时改为返回nil解决
